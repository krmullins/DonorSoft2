<?php
			...
			$schema = [
				...
				'Donations' => [
					...
					'Description' => [
						...
						'info' => [
							'caption' => 'Description',
							'description' => 'How would you describe this donation?',
						],
					],
					'SupporterID' => [
	...
	*/
	...
	function calculated_fields() {
		...
		return [
			'Supporters' => [
				'TotalDonated' => 'SELECT CONCAT(\'$\', FORMAT(COALESCE(SUM(`Donations`.`Amount`), 0.0), 2)) AS `Total`
					FROM `Supporters`
					
					LEFT JOIN `Donations` ON `Donations`.`SupporterID`=`Supporters`.`ID`  
					 
					WHERE `Supporters`.`ID`=\'%ID%\'',
				'TotalMatched' => 'SELECT CONCAT(\'$\', FORMAT(COALESCE(SUM(`MatchingFunds`.`Amount`), 0.0), 2)) AS `Total`
					FROM `Supporters`
			...
			'Campaigns' => [],
			'Donations' => [
				'DonationYear' => 'SELECT YEAR(DonationDate) FROM Donations
		...
		*/
		$lookupQuery = [
			...
			'Donations' => [
				...
				'MailingName' => 'SELECT `Supporters`.`ID`, `Supporters`.`MailingNameFull` FROM `Supporters` ORDER BY 2',
				'Address1' => 'SELECT `Supporters`.`ID`, `Supporters`.`Address1` FROM `Supporters` ORDER BY 2',
				'Address2' => 'SELECT `Supporters`.`ID`, `Supporters`.`Address2` FROM `Supporters` ORDER BY 2',
				'City' => 'SELECT `Supporters`.`ID`, `Supporters`.`City` FROM `Supporters` ORDER BY 2',
				'State' => 'SELECT `Supporters`.`ID`, `Supporters`.`State` FROM `Supporters` ORDER BY 2',
				'Zip' => 'SELECT `Supporters`.`ID`, `Supporters`.`Zip` FROM `Supporters` ORDER BY 2',
				'Country' => 'SELECT `Supporters`.`ID`, `Supporters`.`Country` FROM `Supporters` ORDER BY 2',
			],
			'Notes' => [
				'SupporterID' => 'SELECT `Supporters`.`ID`, IF(CHAR_LENGTH(`Supporters`.`LastName`) || CHAR_LENGTH(`Supporters`.`MailingName`), CONCAT_WS(\'\', `Supporters`.`LastName`, \', \', `Supporters`.`MailingName`), \'\') FROM `Supporters` ORDER BY 2',
			],
